...
Basemap1 <- basemap(limits = c(-72, -48.8, -57, -33.8), bathy.style = "rcb", rotate = TRUE, grid.col = NA, land.col = "transparent", land.border.col = "transparent" ) + 
  theme(legend.position = "right")
  # annotation_scale(location = "br")  
# annotation_north_arrow(location = "tr", which_north = "true") 
Basemap1

pdf("Basemap.pdf", width = 9, height = 4.5)
Basemap1
dev.off()
...
##############################################################################################################################################
## ## Read in processed data with distance calculations from the above
##############################################################################################################################################
...
Cols <- Basemap1 +    
  ...
  ggspatial::geom_spatial_point(data = SA_cols1, aes(x = lon, y = lat, fill = Ordered), size = 2 , shape = 17, colour = "darkgreen") +
...
Fig_1 <- Basemap1 + 
  ...
  guides(fill ="none") +
  ggnewscale::new_scale_color() +
   ggspatial::geom_spatial_point(data = AtSeaOnly, aes(x = lon, y = lat, colour = OnLand), size = 0.0001, colour = "grey20", shape =".") +
  ggspatial::geom_spatial_point(data = ConnectOnly, aes(x = lon, y = lat, colour = OnLand), size = 0.2, colour = "gold") +
  facet_wrap(~Ordered, nrow = 1) +
  geom_polygon(data = Falklands_df, 
               aes(x = x, y = y, group = group), fill = "grey80", colour = "black", linewidth = 0.2, alpha = 0.8) +
  ggspatial::geom_spatial_point(data = SA_cols1, aes(x = lon, y = lat, fill = Ordered), size = 2 , shape = 17, alpha =0.8, colour = "darkgreen") +
  # geom_text(data = SA_cols1, aes(x = lon - 1.4, y = lat, label = ColID),
  #           color = "black", size = 3) +
  xlab("Longitude") + ylab("Latitude") +
  ggspatial::geom_spatial_point(data = RestOnly, aes(x = lon, y = lat, colour = OnLand), size = 0.3, colour = "red") +
    theme(strip.background = element_blank(),
    strip.text.x = element_blank()) +
  theme(legend.position = "none") + labs(colour = "Resting") + guides(fill ="none") 
Fig_1
...
Fig_S3 <- Basemap1 + 
  ...
  ggspatial::geom_spatial_point(data = SAFS_Plot, aes(x = lon, y = lat, colour = id), size = 0.1, shape = 1, alpha = 0.7) +
  facet_wrap(~Group1) +
  scale_colour_viridis_d(direction = -1) +
  ggspatial::geom_spatial_point(data = SAFS_cols1, aes(x = lon, y = lat, fill = Ordered), size = 2 , shape = 17, colour = "darkgreen") +
  ggspatial::geom_spatial_point(data = SAFSRestOnly, aes(x = lon, y = lat), colour = "red", size = 0.6) +
   theme(strip.background = element_blank(),
         strip.text.x = element_blank()) +
...
tiff("Fig_S3.tiff", width = 9, height = 4.5, units = "in", res = 400)
Fig_S3
dev.off()

pdf("Fig_S3.pdf", width = 9, height = 4.5)
Fig_S3
dev.off()
...
tiff("Fig_S2.tiff", width = 6, height = 9, units = 'in', res = 300)
FigS2A
dev.off()

pdf("Fig_S2.pdf", width = 7, height = 5)
FigS2A
dev.off()
...
##############################################################################################################################################
## Create Fig.S1
## Showing tracking data for the 6 species filtered out of the analysis
##############################################################################################################################################
...
Fig_New <- Basemap1 + 
  ...
  guides(fill ="none") +
  ggnewscale::new_scale_color() +
  ggspatial::geom_spatial_point(data = AllSup, aes(x = lon, y = lat, group = Group, colour = SPP), size = 0.05, alpha = 0.5) +
...
pdf("Fig_S1.pdf", width = 6, height = 8)
Fig_New
dev.off()
